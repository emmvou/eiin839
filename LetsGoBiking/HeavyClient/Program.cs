...
namespace HeavyClient
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine($"{client.GetAllStations()} took {DateTime.Now.Subtract(t).TotalMilliseconds,10:F3}ms to compute");
            t = DateTime.Now;
            Console.WriteLine($"{client.GetAllStations()} took {DateTime.Now.Subtract(t).TotalMilliseconds,10:F3}ms to compute");
            //t = DateTime.Now;
            ...
            //Console.WriteLine($"{client.GetRoute(45.732398, 4.835571, 45.85909499964853, 4.748162441997923, "lyon"),10} took {DateTime.Now.Subtract(t).TotalMilliseconds}ms to compute");
            t = DateTime.Now;
            Console.WriteLine($"{client.GetStation(7005, "lyon").ToString()} took {DateTime.Now.Subtract(t).TotalMilliseconds,12:F3}ms to compute");
            t = DateTime.Now;
            Console.WriteLine($"{client.GetStation(7005, "lyon").ToString()} took {DateTime.Now.Subtract(t).TotalMilliseconds,12:F3}ms to compute");
            Console.WriteLine("pause - 30 seconds");
            Thread.Sleep(30000);
            t = DateTime.Now;
            Console.WriteLine($"{client.GetStation(7005, "lyon").ToString()} took {DateTime.Now.Subtract(t).TotalMilliseconds,12:F3}ms to compute");
            t = DateTime.Now;
